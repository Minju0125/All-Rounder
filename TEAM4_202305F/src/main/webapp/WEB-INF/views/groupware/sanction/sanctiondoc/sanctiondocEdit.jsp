...
<script>
	const firstCode = "${editSanction.sanctnRcyer }";
	const firstRec = "[${editSanction.sanctnRcyerDeptName}]${editSanction.sanctnRcyerNm} ${editSanction.sanctnRcyerRankNm}";
...
<!-- Content -->
	<div class="container-xxl flex-grow-1 container-p-y">
		...
		<button class="btn btn-sm btn-outline-secondary" id="proxyBtn">임시저장</button>
		 -->
		<button class="btn btn-sm btn-outline-secondary" onClick="history.back()" id="listBtn">이전</button>
		</div>
		<div class="row">
		<!-- User Content -->
					...
					<div class="card-body">
						<h2 class="card-header">${formSet.formNm }</h2>
							<div class="table-responsive mb-3">
								<form:form id="sanctionForm" enctype="multipart/form-data" modelAttribute="editSanction" >
								<input type="hidden" name="sanctnNo" id="sanctnNo" value="${editSanction.sanctnNo }">
								<input type="hidden" name="sanctnSttus" id="sanctnSttus" value="${editSanction.sanctnSttus }">
								<table>
									<tbody>
										<tr>
											<td class="title-cell">
										...
										<tr>
											...
											<td>
												<input type="text"	name="sanctnDate" id="sanctnDate" style="width: 100%" value="${editSanction.sanctnDate }" readonly/>
											</td>
										</tr>
										<tr>
											<td class="title-cell">
								...
								<br />
								<div class="mb-3">
									<textarea rows="20"	cols="50" name="sanctnSourc" id="sanctnSourc">${editSanction.sanctnSourc}</textarea>
								</div>
							</form:form>
						</div>
					</div>
				...
				<div class="card mb-4">
					<div class="card-body first">
						...
						<form>
							...
							<c:if test="${not empty editSanction.lineList }">
								<c:forEach var="sanctionLineVO" items="${editSanction.lineList}">
									<p>${sanctionLineVO.sanctnerNm} ${sanctionLineVO.sanctnerRankNm} </p>
								</c:forEach>
							</c:if>
							</div>
						</form>
...
<script type="text/javascript">
	...
	var sanctnNo = ${editSanction.sanctnNo};
	
</script>
<script src="${pageContext.request.contextPath }/resources/js/app/sanction/sanctionDocEdit.js"></script>
