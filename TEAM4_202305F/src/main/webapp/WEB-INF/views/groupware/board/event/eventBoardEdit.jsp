...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
  <div class="d-flex flex-column justify-content-center">
    <h4 class="mb-1 mt-3">경조사게시판수정</h4>
    <p class="text-muted">Edit a Notice Board</p>
  </div>
</div>
  ...
  <security:csrfInput />
    <div class="row g-2">
      <div class="col-md-8 col-lg-8 mb-3">
        ...
        <div class="input-group has-validation" >
	        <input type="text" name="bbsSj"  id="bbsSj" class="form-control"  value="${detail.bbsSj }" />
        </div>
      </div>
        ...
        <label for="eventSttus" class="fw-medium"><i class="bx bx-calendar-exclamation bx-sm mb-1"></i>
         ...
         <div class="input-group has-validation" >
	        <select name="eventSttus" name="eventSttus" id="eventSttus" class="form-select" required>
	          ...
	          <option value="1" <c:if test="${detail.eventSttus eq '1'}">selected</c:if>>결혼/출산</option>
	          <option value="2" <c:if test="${detail.eventSttus eq '2'}">selected</c:if>>사망</option>
	          <option value="3" <c:if test="${detail.eventSttus eq '3'}">selected</c:if>>생일</option>
	          <option value="4" <c:if test="${detail.eventSttus eq '4'}">selected</c:if>>기타</option>
	        </select>
        </div>  
      </div>
    </div>
    <div class="row g-2">
      <div class="col-md-5 col-lg-5 mb-3">
        <label for="eventBdate" class="fw-medium"><i class="bx bx-calendar-edit bx-sm mb-1"></i> 시작일</label>
        <div class="input-group has-validation" >
       	 <input name="eventBdate" value="${detail.eventBdate}" type="date" id="eventBdate" class="form-control"  />
	    </div>
      </div>
      <div class="col-md-2 col-lg-2 mb-3" style="padding-top: 33px; padding-left: 20px;">
       <label for="dobBackdrop" class="fw-medium"><i class="bx bx-calendar-exclamation bx-sm mb-1"></i>종일</label>
        <input type="checkbox"  
        	<c:if test="${detail.eventBdate eq detail.eventEdate}">checked</c:if>
        		class="form-check-input" name="allDay" id="allDay" />
      </div>
      <div class="col-md-5 col-lg-5 mb-3">
        <label for="eventEdate" class="fw-medium"><i class="bx bx-calendar-exclamation bx-sm mb-1"></i>
          종료일</label>
         <div class="input-group has-validation">
             <input type="date" id="eventEdate" name="eventEdate" class="form-control rounded-1"  value='<c:out value="${detail.eventEdate}"/>'>
         </div> 
      </div>
    </div>
    
    <div class="mb-3">
      <label for="bbsCn" class="fw-medium"><i class="bx bx-calendar-exclamation bx-sm mb-1"></i>
        내용</label>
        <textarea name="bbsCn" id="bbsCn" rows="5" cols="10"  class="form-control">${detail.bbsCn}</textarea>
    </div>
    <div class="row g-2">
      <div class="col mb-3">
        <label for="dobBackdrop" class="fw-medium"><i class="bx bx-calendar-edit bx-sm mb-1"></i> 주소</label>
        <input name="eventLoc" value="${detail.eventLoc}" type="text" id="eventLoc" class="form-control" />
      </div>
      <div class="col mb-3">
    ...
    <input name="bbsNo"  value="${detail.bbsNo}" type="hidden" id="originBbsEquator" class="form-control" />
    <input name="bbsEquator"  value="${detail.bbsEquator}"  type="hidden" id="bbsEquator" class="form-control" />
    <input name="bbsLatitude" value="${detail.bbsLatitude}"  type="hidden" id="bbsLatitude" class="form-control" />
    ...
    <button type="button" class="btn btn-primary me-1" id="updBtn" onclick="fn_updateEvent()">수정</button>
    <button type="button" class="btn btn-info me-1"
      onclick="location.href='<c:url value='/event/home' />'">목록</button>
    <button type="reset" class="btn btn-label-secondary me-1" id="reset" onclick="location.href='<c:url value='/event/detail?bbsNo=${detail.bbsNo}&eventSttus=${detail.eventSttus}' />'">취소</button>
  </form>
</div>
