...
<html
  ...
  <head>
    ...
    <script src="/resources/js/jquery-3.7.1.min.js"></script>
  	...
  	<!-- SweetAlarm -->
  	<script src="/resources/assets/vendor/libs/sweetalert2/sweetalert2.js"></script>
  	<link rel="stylesheet" href="/resources/assets/vendor/libs/sweetalert2/sweetalert2.css" />
	...
	<script>
		...
		console.log("SERVERSIDEADDRESS : " + SERVERSIDEADDRESS);
		console.log("CSRFPARAMNAME : " + CSRFPARAMNAME);
            ...
            <div class="card-body">
              ...
              <div class="app-brand justify-content-center">
                <a href="index.html" class="app-brand-link gap-1">
                  ...
                  <span class="app-brand-text demo text-body fw-bold">All-Rounder</span>
                </a>
              </div>
              <!-- /Logo -->
              <h4 class="mb-2">비밀번호 변경 🔒</h4>
              <p class="mb-4">비밀번호 변경 후에 사용이 가능합니다.<br>비밀번호를 6~15자 이내로 입력하세요.</p>
			 <!-- ajax 로 받은 데이터 올 자리 -->
  			  <div id="myContent">
	  			  <form method="post" class="mb-3" action="/checkCertiNum"
						enctype="application/x-www-form-urlencoded" id="certiNumForm">
				<div class="mb-3">
					<label for="empPw" class="form-label">변경할 비밀번호</label>
					<input type="text" class="form-control" id="empPw" name="empPw" placeholder="새로운 비밀번호를 입력해주세요." autofocus />
					<input type="hidden" name="empCd" id="empCd" value="${empCd}"/>
				</div>
				<input type="button" class="btn btn-primary d-grid w-100" value="확인" id="modifyBtn">
				<sec:csrfInput/>
			</form>
  			  </div>
            </div>
    ...
    <script>
        ...
        const employee = {
          ...
          empPw : $("#empPw").val()
    		...
    		$.ajax({
    			type : "PUT",
    			url : `/findPw/modifyPasswordProcess`,
          ...
          success : function(resp){
            if(resp.success=="isNotValid"){ //형식에 맞지 않는 경우
            	alert("안마쟈")
              Swal.fire("형식에 맞게 입력해주세요.", "", "error").then((result)=>{
            	 location.reload();
              });
              location.reload();
            }else if(resp.success=="OK"){ //변경 완료인 경우
              //비밀번호가 변경되었습니다. 다시 로그인해주세요 !
              Swal.fire("비밀번호가 변경되었습니다.", "변경된 비밀번호로 다시 로그인해주세요.", "success").then((reuslt)=>{
               	//세션 만료하고 로그인 페이지로
            	  location.href = "/logout";
              });
            }else{
              Swal.fire("비밀번호 변경에 실패하였습니다.", "다시 시도해주세요.", "error").then((reuslt)=>{
                location.reload();
              });
            }
          },
